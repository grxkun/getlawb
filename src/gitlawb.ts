...
export class GitlawbIntegration {
  ...
  constructor(client: GetlawbClient, config: GitlawbConfig) {
    ...
    this.privateKey = this.loadKey(pem);
  }
  ...
  /**
   * Load an Ed25519 private key from PEM, handling both standard PKCS#8 and
   * the OneAsymmetricKey (RFC 5958 v1) format generated by the gitlawb CLI.
   */
  private loadKey(pem: string): crypto.KeyObject {
    try {
      return crypto.createPrivateKey(pem);
    } catch {
      // gitlawb CLI generates OneAsymmetricKey (version 1 with embedded public key).
      // Convert: extract the 32-byte seed and rebuild as standard PKCS#8 (version 0).
      ...
      const stdPem =
        ...
        '\n-----END PRIVATE KEY-----\n';
      return crypto.createPrivateKey(stdPem);
    }
  }
